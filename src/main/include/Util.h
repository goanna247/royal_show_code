...
// Copied from Wombat Library

namespace wom {
  template<typename T>
  /**
   * @brief easy inverting of systems without worrying about types
   * @param system the system to invert
  */
  T&& invert(T &&system) {
  ...
  */
   ...
   * @param val the value to apply a deadzone to
   * @param deadzone the deadzone magnitude. Defaults to 0.05
   * @returns the deadzoned value
  */
  double deadzone(double val, double deadzone = 0.05);
  /**
   * @brief squares a value, but keeps the sign
   * @param val the value to square
   * @returns the squared value with the original sign
  */
